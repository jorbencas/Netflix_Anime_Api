<?php
  function User($api) {
    switch ($api->getAction()) {
      case 'Login': $result = Login($api); break;  
      case 'Register': $result = Register($api); break;  
      case 'logout': $result = $api->isValidAccesToken() ? logout($api) : $api->response("api_Auth_resp_error_msg", 404); break;  
      case 'updateUser': $result = $api->isValidAccesToken() ? updateUser($api) : $api->response("api_User_resp_error_msg", 404);  break;
      case 'apione': $result = /*$api->isValidAccesToken() ?*/ getOneUser($api) /*: $api->response("api_User_resp_error_msg", 404)*/; break;
      case 'apiby': $result = $api->isValidAccesToken() ? getalluser($api) : $api->response("api_User_resp_error_msg", 404);  break;
  ...
  function Register($api) {
    ...
    if (!isset($POST['username']) || !isset($POST['passwd'])) {
      ...
      $password = isset($POST['passwd']) ? $POST['passwd'] : "Karanlik123?";
