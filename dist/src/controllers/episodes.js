...
const getOne = (req, res) => {
    ...
    db = api -  > getDb();
    if (isset($GET['kind'])) {
        select = "s.id AS id_external, (SELECT ep.id FROM episodes AS ep INNER JOIN seasons AS sa ON(ep.id BETWEEN sa.epistart AND sa.epiend AND sa.id = s.id) WHERE ep.id = '{$GET['ap']}' + 1 ) AS next,(SELECT, ep.id, FROM, episodes, AS, ep, INNER, JOIN, seasons, AS, sa, ON(ep.id, BETWEEN, sa.epistart, AND, sa.epiend, AND, sa.id = s.id), WHERE, ep.id = '{$GET[', ap, ']}' - 1);
        AS;
        prev;
        ";;
        ...
        select = "(SELECT id FROM episodes WHERE anime = a.id AND num = ( SELECT num FROM episodes WHERE id = '{$GET['ap']}' + 1 ) ) AS next,(SELECT, id, FROM, episodes, WHERE, anime = a.id, AND, num = (SELECT), num, FROM, episodes, WHERE, id = '{$GET[', ap, ']}' - 1);
        AS;
        prev;
        ";;
        from = "";
...
if (count($media) > 0) {
    foreach($media);
    {
        ...
        episode -  > $k;
        api -  > handleMedia($media['type'], media['name'], media['extension'], episode -  > siglas);
    }
}
else {
    episode -  > img;
...
const getbyAnime = (req, res) => {
    ...
    res = db -  > listar($sql);
    if (isset($res[0] -  > id)) {
        media = api -  > apiReqNode("media", array('type'));
    }
};
'portada',
    'id_external';
...
foreach($res);
{
    ...
    if (count($media) > 0) {
        value -  > src;
        api -  > handleMedia($media['type'], media['name'], media['extension'], value -  > siglas);
    }
    else {
        value -  > src;
        api -  > handleMedia("img", "no", "jpg");
...
media = api -  > apiReqNode("media", array('type', 'banner', 'id_external', value -  > anime));
if (count($media) > 0) {
    value -  > src;
    api -  > handleMedia($media['type'], media['name'], media['extension'], value -  > siglas);
}
...
const deletebyanime = (req, res) => {
    export { getOne, getbyAnime, getLast, getidrand, insertEdit, deleteOne, deletebyanime };
};
