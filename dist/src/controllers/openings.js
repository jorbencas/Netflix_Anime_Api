...
const getOne = (req, res) => {
    db = api -  > getDb();
    GET = api -  > getGET();
    sql = "SELECT o.id, o.anime, o.nombre, o.descripcion, a.siglas, o.num,(SELECT, id, FROM, openings, WHERE, anime = o.anime, AND, num = (SELECT), num, FROM, openings, WHERE, id = '{$GET[', ap, ']}' + 1);
    AS;
    next,
        (SELECT);
    id;
...
if (count($media) > 0) {
    foreach($media);
    {
        ...
        opening -  > $k;
        api -  > handleMedia($media['type'], media['name'], media['extension'], opening -  > siglas);
    }
}
else {
    opening -  > img;
...
const insertEdit = (req, res) => {
    ...
    if (!isset($valor[0] -  > id)) {
        ...
        descripcion, anime, num;
        VALUES('{$POST[', id, ']}', '{$POST[', nombre, ']}', '{$POST[', descripcion, ']}', '{$POST[', anime, ']}', '{$POST[', num, ']}');
        ";;
        inserted = db -  > ejecutar($sql);
        if ($inserted) {
            api -  > writeFile(array("kind"));
        }
    }
};
exports.insertEdit = insertEdit;
"Opening insertado", "message";
"Se ha insertado el opening {$POST['nombre']}";
"log";
;
